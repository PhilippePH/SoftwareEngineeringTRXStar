        ...
        <div>
            ...
            <div className={isFadingOut ? 'item-fadeout': (isSlidingIn ? 'slide-in': (isSlidingUp ? 'flipup': (isSlidingDown ? 'flipdown': 'item1')))}>
                ...
                <div>
                    ...
                    {(exercise_name == "Warmup" || exercise_name == "Cooldown")&& type != "rest"&&<div className='warmup-cooldown-container'> Total Duration: {Math.floor((sets*duration+(time*(sets-1)))/60)}:{remaining_secs_duration<10?'0':''}{remaining_secs_duration}</div>}
                    {exercise_name != "Warmup" && exercise_name != "Cooldown" &&type != "rest"&& <div className='info-container'> 
